    ...
    <GuestPage>
      ...
      <section className="py-10">
        <div className="container grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
          <form action={submitGuestbookAction} className="section-frame space-y-4 p-6">
            <input type="hidden" name="token" value={params?.token || ""} />
            <HeartHandshake className="text-[#9b7039]" />
            <h2 className="serif text-4xl font-semibold">Sign the guestbook</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {entries.map((entry) => (
              <article key={entry.id} className="section-frame p-5">
                <p className="script text-2xl text-[#6e5d52]">“{entry.message}”</p>
