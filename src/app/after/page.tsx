...
export default async function AfterWeddingPage() {
  ...
  return (
    <GuestPage>
      <section className="relative overflow-hidden bg-[#15110f] py-14 text-center text-white sm:py-20">
        ...
        <div className="container relative max-w-4xl">
          ...
          <h1 className="serif mt-5 text-5xl font-semibold uppercase tracking-[0.06em] sm:text-8xl sm:tracking-[0.08em]">Thank you</h1>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-[#f3e7da] sm:text-lg sm:leading-8">{setting.afterWeddingMessage || "Thank you for surrounding us with so much love. We are grateful for every hug, toast, dance, prayer, and memory from our wedding weekend."}</p>
      ...
      <section className="py-8 sm:py-10">
        <div className="container grid gap-5 lg:grid-cols-3">
          <div className="section-frame p-5 sm:p-6 lg:col-span-2">
            <h2 className="serif text-3xl font-semibold uppercase tracking-[0.06em] sm:text-4xl sm:tracking-[0.08em]">Gallery highlights</h2>
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              {photos.map((photo) => <img key={photo.id} src={photo.imageUrl} alt={photo.title} loading="lazy" decoding="async" className="h-52 w-full object-cover sm:h-56" />)}
            </div>
          ...
          <div className="section-frame p-5 sm:p-6">
            ...
            <h2 className="serif mt-4 text-3xl font-semibold sm:text-4xl">Registry links</h2>
        ...
        <div className="container mt-5 grid gap-4 md:grid-cols-4">
          {guestbook.map((entry) => (
            <article key={entry.id} className="section-frame p-5">
              <p className="script text-xl leading-snug sm:text-2xl">“{entry.message}”</p>
